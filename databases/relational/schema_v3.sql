...
CREATE TABLE users (
    user_id         VARCHAR(10)  PRIMARY KEY REFERENCES users(user_id) ON DELETE CASCADE, -- natural key from mock data e.g. "RU01"
...
CREATE TABLE metro_stations (
    ...
    interchange_nr_station_id VARCHAR(10) REFERENCES national_rail_stations(station_id) ON DELETE SET NULL,
);
...
CREATE TABLE national_rail_schedules (
    schedule_id             VARCHAR(20)  PRIMARY KEY,
    -- natural key from mock data e.g. "NR_SCH01"
    line                    VARCHAR(10)  NOT NULL,
    ...
    origin_station_id       VARCHAR(10)  NOT NULL REFERENCES national_rail_stations(station_id)
                            ON DELETE RESTRICT,
                            -- RESTRICT: cannot delete a station used as a schedule origin
    destination_station_id  VARCHAR(10)  NOT NULL REFERENCES national_rail_stations(station_id)
                            ON DELETE RESTRICT,
                            -- RESTRICT: cannot delete a station used as a schedule destination
    passed_through_stations JSONB,
                            -- informational only: stations train passes without stopping
                            -- kept as JSONB since no query filters by individual pass-through stations
    first_train_time        TIME         NOT NULL,
    last_train_time         TIME         NOT NULL,
...
-- Stop sequence + travel times for national rail schedules
-- Replaces stops_in_order and travel_time_from_origin JSONB columns
CREATE TABLE national_rail_schedule_stops (
    schedule_id      VARCHAR(20) NOT NULL REFERENCES national_rail_schedules(schedule_id)
                     ON DELETE CASCADE,
                     -- CASCADE: deleting a schedule removes all its stops
    station_id       VARCHAR(10) NOT NULL REFERENCES national_rail_stations(station_id)
                     ON DELETE RESTRICT,
                     -- RESTRICT: cannot delete a station that is an active stop
    stop_order       INT         NOT NULL, -- 0 = origin, 1 = next stop, etc.
    travel_time_min  INT         NOT NULL, -- cumulative minutes from origin
    PRIMARY KEY (schedule_id, station_id)
);
...
CREATE TABLE coaches (
    coach_id   SERIAL      PRIMARY KEY, -- surrogate key: coaches have no natural ID
...
CREATE TABLE seats (
    seat_pk    SERIAL      PRIMARY KEY, -- surrogate key: seat_id is only unique within a coach
